...
- **`hypothesis_segment_links`**: Stable relationship between a hypothesis and a segment (one row per pair) storing the **latest** analysis state.
    - `id`: Stable UUID for the hypothesis↔segment pair.
    ...
    - `updated_at`: When the latest analysis for this pair was last saved.
- **`hypothesis_segment_link_runs`**: Append-only run log (history) of saved analyses for a hypothesis↔segment pair.
    - `link_id`: FK to `hypothesis_segment_links.id`.
    - Includes snapshots of the hypothesis fields at save-time for auditability.
...
- **`question_hypothesis_links`**: Many-to-many link between questions and hypotheses.
...
Questions (optional navigation)
  ...
  link to
     ↓
Hypotheses ←── hypothesis_segment_links ←── Segments
  (core)               (latest state)        (evidence)

            hypothesis_segment_link_runs
                  (history log)
```
...
- **Workflow:**
    ...
    4. **Save Evidence:**
        ...
        - Saving updates `hypothesis_segment_links` (latest state) and appends a new row to `hypothesis_segment_link_runs` (history).
